...
/////////////////////////////////////////
    ...
    // Sensitivity = -2 to 2 G at 16Bit -> 2G = 32768 && 0.5G = 8192
		...
		_delay_ms(5);
		uart_putstring("E");		//first character of serial terminal transmission
		uart_putdouble(gyroZ);		//display gyro data of Z axis
		uart_putstring(",");		//separator
		uart_putdouble(pitch);		//display pitch angle
		uart_putstring(",");		//separator
		uart_putdouble(roll);		//display roll angle
		uart_putstring(",");		//separator
		uart_putdouble(gyroX);		//OPTIONAL display gyro data of X axis
		uart_putstring(",");		//separator
		uart_putdouble(gyroY);		//OPTIONAL display gyro data of Y axis
		uart_putstring("\n");		//end of string for input values
		uart_putstring("                                                                  X-axis turns: ");
		uart_putdouble(pitch_count);	//number of pitches
		uart_putstring("        Y-axis turns: ");
		uart_putdouble(roll_count);		//number of rolls
		uart_putstring("\n");
